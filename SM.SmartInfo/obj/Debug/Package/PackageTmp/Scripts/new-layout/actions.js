...
function page_content_onresize() {
    ...
    //todo: haidv fix loi thua content duoi footer
    var content_minus = 0;
    content_minus = ($(".page-container-boxed").length > 0) ? 40 : content_minus;
    //content_minus += ($(".page-navigation-top-fixed").length > 0) ? 50 : content_minus;
    content_minus += ($(".page-navigation-top-fixed").length > 0) ? 0 : content_minus;
    ...
    if (content.height() < $(document).height() - content_minus) {
        ...
        min_height = $(document).height() - content_minus;
        content.attr('style', 'min-height:' + min_height + 'px');
    }

    if (sidebar.height() > content.height() && sidebar.height() > $(document).height() - content_minus) {
        //content.height(sidebar.height());        
        min_height = sidebar.height();
        //content.attr('style', 'min-height:' + min_height + 'px');
    }
...
function build_menu_onresize() {
    var $menu = $('.x-navigation-horizontal');
    var w = $menu.width();
    var profilesWidth = $menu.find('li.user-name').first().width();
    var $hoverMenu = $menu.find('li.button-link').first();
    var $contentHover = $hoverMenu.find('div.mCSB_container').first();
    ...
    $menu.find('>li:not(li.user-name, li.button-link)').each(function () {
        dynamicWidth += $(this).width();
        //10px padding voi profiles
        if (dynamicWidth > w - profilesWidth - 10) {
            arrMenu.push(indexItem);
            strText += "<li class='list-group-item " + $(this).prop("classList") + "'>" + $(this).html(); + "</li>";
            $(this).css({
                display: 'none'
            });
        } else {
    ...
    if (arrMenu.length > 0) {
        $contentHover.html(strText);
        //100px = header + footer
        $hoverMenu.find('div.list-group').first().css({
