...
namespace SM.SmartInfo.Utils
{
    public static class Utility
    {
        ...
        public static void Decrypt(string cypherText, out int? empID, out int[] refID)
        {
            ...
            string plainText = Decrypt(cypherText);
        ...
        public static string GetString(decimal? value)
        {
            ...
            {
                if ((value.Value - Math.Round(value.Value) != 0))
                    return SoftMart.Core.Utilities.Utility.GetDecimalString(value.Value, 4);
                else
                    return SoftMart.Core.Utilities.Utility.GetDecimalString(value.Value, 0);
            }
        }
        ...
        public static string GetString(decimal? value, int numberDigit)
        {
            if (value == null)
                ...
                return SoftMart.Core.Utilities.Utility.GetDecimalString(value.Value, numberDigit);
        }
        #endregion
        ...
        {
            return SoftMart.Core.Utilities.Utility.ConvertToDecimal(value);
        }
        ...
        #region Image
        public static Tuple<decimal, decimal> GetLatLngFromByteImage(byte[] fileContent)
        {
            try
            {
                System.IO.MemoryStream ms = new System.IO.MemoryStream(fileContent);
                System.Drawing.Image image = System.Drawing.Image.FromStream(ms);
                return GetLatLngFromImage(image);
            }
            catch
            {
                return null;
        ...
        public static Tuple<decimal, decimal> GetLatLngFromImage(System.Drawing.Image image)
        {
            try
            {
                ...
                {
                    decimal latitude = DecodeRational64u(image.GetPropertyItem(2));
                    decimal longitude = DecodeRational64u(image.GetPropertyItem(4));
        ...
        private static decimal DecodeRational64u(System.Drawing.Imaging.PropertyItem propertyItem)
        {
        ...
        #region Encrypt
        public static string EncryptParam(params object[] arrParam)
        {
        ...
        public static string Encrypt(string plainText, string defaultPassword = Password)
        {
            // Check arguments. 
        ...
        #region Decrypt
        public static List<string> DecryptParam(string cypherText)
        {
            if (string.IsNullOrWhiteSpace(cypherText))
        ...
        public static string Decrypt(string encryptedText, string defaultPassword = Password)
        {
            // Check arguments.
        ...
        #region Base64
        public static string EncryptCode64Bit(string text)
        {
            if (!string.IsNullOrWhiteSpace(text))
        ...
        public static string DecryptCode64Bit(string text)
        {
            if (!string.IsNullOrWhiteSpace(text))
        ...
        public static Dictionary<int?, string> CreateTypeDictionary(PressAgencyParam param)
        {
            var typeDic = new Dictionary<int?, string>();
            foreach (var item in param.ListAgencyType)
            {
                typeDic.Add(item.Id, item.TypeName);
            }
            return typeDic;
        }
    }
